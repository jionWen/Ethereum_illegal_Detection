...
def get_normal_account_addresses():
    import pandas as pd
    csv_file = '../Data_processing/block.csv' #利用工具获取的本地数据
    df = pd.read_csv(csv_file)
    senders = df.head(3000)
    receivers = df.tail(3000)
    s = np.unique(senders['s'].values)
    r = np.unique(receivers['r'].values)
    good_accounts = np.concatenate((s,r), axis=0)
    unique_good_accounts = np.unique(good_accounts)  #账户去重
    print("合法账户的数量 ", len(unique_good_accounts))
...
def account_balance(address):
    url = "https://api.etherscan.io/api?module=account&action=balance&address={address}" \
          "&tag=latest&apikey=SMQ32N54NBHSGW49NRM1MRJB8B3S9DTXTP".format(address=address)
...
def get_total_number_of_normal_transactions(address):
    ...
    url = "http://api.etherscan.io/api?module=account&action=txlist&address={address}" \
          "&startblock=0&endblock=99999999&sort=asc&apikey=SMQ32N54NBHSGW49NRM1MRJB8B3S9DTXTP".format(address=address)
    r = requests.get(url=url)
    data = r.json()
...
def token_transfer_transactions(address):
    URL = "http://api.etherscan.io/api?module=account&action=tokentx&address={address}" \
          "&startblock=0&endblock=999999999&sort=asc&apikey=SMQ32N54NBHSGW49NRM1MRJB8B3S9DTXTP".format(address=address)
    r = requests.get(url=URL)
    data = r.json()
    #print(data)
    #初始化从获取的交易数据的字段即交易特征，这里使用了42个特征
...
def normal_transactions(index, address, flag):
    URL = "https://api.etherscan.io/api?module=account&action=txlist&address={address}" \
          "&startblock=0&endblock=99999999&page=1&offset=100000&sort=asc&apikey=SMQ32N54NBHSGW49NRM1MRJB8B3S9DTXTP".format(address=address)
